...
from arc import themed_style
from version import *

#APPLICATION NAME HANDLE 
appName = 'Application Name'
#TITLE BAR AND FRAME THEME
root = tk.Tk()
root.title(appName + ' ' + appVersion)
style = themed_style()

#MAINFRAME TITLE AND CAPTION
frame = ttk.Frame(root, padding=25)
frame.pack(fill='both', expand='yes')

ttk.Label(frame, text='ARC Theme', font='-size 30', anchor='center').pack(fill='x')
ttk.Label(frame, text='Here is some text... and a place to enter text', anchor='w').pack(fill='x', pady=10)

#TEXTBOX
entry = ttk.Entry(frame)
entry.pack(fill='x', pady=5)
entry.insert('end', 'This a text')
#DISABLES EDITING THE FIELD (COMMENT OUT IF NOT NEEDED)
